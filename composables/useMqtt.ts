...
import { decryptMessage, importPublicKey } from "~/utils/crypto";
import {
...
export function useMqtt() {
	...
	async function importAndStorePeerKey(
	...
	async function handleMessage(topic: string, payload: Buffer) {
		...
		if (topic === topics.messages) {
			try {
				const parsed = EncryptedMessageSchema.safeParse(
				...
				if (!senderPublicKey) {
					senderPublicKey = await importAndStorePeerKey(
						encryptedMsg.senderPublicKey,
					);
					if (senderPublicKey) {
						store.updatePeer({
			...
			try {
				const parsed = PublicKeyAnnouncementSchema.safeParse(
					JSON.parse(payload.toString()),
				);
				if (!parsed.success) {
					if (import.meta.dev) {
						console.error(
							"Invalid public key announcement schema:",
							parsed.error,
						);
					}
					return;
			...
			try {
				const parsed = PublicKeyRequestSchema.safeParse(
					JSON.parse(payload.toString()),
				);
				if (!parsed.success) {
					if (import.meta.dev) {
	...
	function announcePublicKey() {
		...
		client.value.publish(store.topics.pubkeys, JSON.stringify(announcement), {
			qos: 0,
		});
	}
	...
	function requestPublicKeys() {
		...
		client.value.publish(store.topics.pubkeyRequest, JSON.stringify(request), {
			qos: 0,
		});
	}
	...
	function sendReceipt(messageId: string, status: ReceiptStatus) {
		...
		client.value.publish(store.topics.receipts, JSON.stringify(receipt), {
			qos: 0,
		});
	}
	...
	async function sendMessage(text: string) {
		...
		const recipientKeys = Array.from(store.peerPublicKeys.values()).map(
			(p) => p.key,
		);
		...
		encryptedByPublicKey[store.myPublicKeyString] =
			encrypted[recipientKeys.length - 1];

		const encryptedMsg: EncryptedMessage = {
			id: `${Date.now()}-${Math.random()
				.toString(36)
				.substring(2, 2 + MESSAGE_ID_RANDOM_LENGTH)}`,
			username: store.displayUsername,
	...
	function emitTypingStatus(isTyping: boolean) {
		...
		client.value.publish(store.topics.typing, JSON.stringify(typingEvent), {
			qos: 0,
		});
	}
