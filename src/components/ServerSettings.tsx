...
export function ServerSettings({ config, onSave, disabled }: ServerSettingsProps) {
  ...
  const handleSave = () => {
    ...
    if (!brokerUrl.trim() || isNaN(portNum) || portNum <= 0 || !topicPrefix.trim()) {
      return
    }
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="bg-card border-border">
        <DialogHeader>
          <DialogTitle className="font-mono text-primary">MQTT Server Settings</DialogTitle>
