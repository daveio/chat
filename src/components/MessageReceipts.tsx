...
export function MessageReceipts({
	receipts,
	totalPeers,
}: MessageReceiptsProps) {
	if (totalPeers === 0) return null;

	const decryptedCount = receipts.filter(
		(r) => r.status === "decrypted",
	).length;
	...
	const getIcon = () => {
		if (decryptedCount === totalPeers) {
			...
			return (
				<Clock size={14} className="text-muted-foreground" weight="fill" />
			);
		}
	};
		...
		return (
			<div className="space-y-2 max-w-xs">
				...
				{decrypted.length + received.length < totalPeers && (
					<p className="text-xs text-muted-foreground">
						Waiting for {totalPeers - decrypted.length - received.length}{" "}
						peer(s)
					</p>
				)}
			</div>
